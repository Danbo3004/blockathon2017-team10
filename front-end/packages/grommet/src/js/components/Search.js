...
export default class Search extends Component {
  ...
  _renderDropContent () {
    const {
      defaultValue, dropAlign, dropColorIndex, suggestions, value, iconSize
    } = this.props;
    ...
    if (!inline) {
      contents = [
        <Button key='icon' icon={<SearchIcon size={iconSize} />}
  ...
  render () {
    const {
      className, defaultValue, iconAlign, id, fill, pad, placeHolder, size, iconSize,
      value
    } = this.props;
    ...
    const classes = classnames(
      CLASS_ROOT,
      {
        ...
        [`${CLASS_ROOT}--icon-size-${iconSize}`]: iconSize
      },
      className
    );
    ...
    if (inline) {
      ...
      return (
        <Button ref={(ref) => this._controlRef = ref}
          id={id} className={className} icon={<SearchIcon size={iconSize} />}
