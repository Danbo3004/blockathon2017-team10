...
export default class TextInput extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (dropActive && !prevState.dropActive) {
      ...
      if (this.componentRef) {
        ...
        this._drop = new Drop(control, this._renderDropContent(), {
          align: { top: 'bottom', left: 'left' },
          responsive: false // so suggestion changes don't re-align
        });
      }
    } else if (dropActive && prevState.dropActive && this._drop) {
  ...
  _onFocus(event) {
    ...
    setTimeout(() => {
      if (this.componentRef) this.componentRef.select();
    }, 10);
    ...
    if (suggestions) {
      items = suggestions.map((suggestion, index) => {
        let classes = classnames({
          ...
          [`${CLASS_ROOT}__suggestion--active`]: index === activeSuggestionIndex
        });
        return (
          <li
            key={index}
  ...
  render() {
    ...
    const inputElem =
      (inputFormat && inputFormat.length) || thousandSeparator ? (
        <NumberFormat
          ...
          onValueChange={data => this._onInputChange({ target: data })}
    ...
    return unit ? (
      <Box direction="row" flex align="center">
        <Box basis="full" className={`${CLASS_ROOT}__input`}>
          {inputElem}
        </Box>
        <Box>
          <span className={`${CLASS_ROOT}__unit`}>{unit}</span>
        </Box>
      </Box>
    ) : (
      inputElem
