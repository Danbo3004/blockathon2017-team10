...
export default class Image extends Component {
  render() {
    ...
    const imgNode = <img {...props} className={classes} />;

    const labelRoot = `${CLASS_ROOT}__caption`;
    const labelClasses = classnames(labelRoot, {
      [`${labelRoot}--${size}`]: size
    });
    return caption && captionText ? (
      <span className={`${CLASS_ROOT}__container`}>
        {imgNode}
        <Label className={labelClasses}>{captionText}</Label>
      </span>
    ) : (
      imgNode
    );
...
Image.propTypes = {
  ...
  mask: PropTypes.bool,
  size: PropTypes.oneOf(['small', 'medium', 'large', 'thumb', 'small-thumb']),
